...
describe("VestingStake", () => {
  ...
  beforeEach(async () => {
    ...
    await axialToken
      .connect(deployer)
      .mints(
        [await deployer.getAddress(), aliceAddr, bobAddr, carolAddr],
        [1000, 10, 100, 500]
      );
    stakingVe = await stakingVeFactory.deploy(
      axialToken.address,
      "sAxial",
      "SAXIAL",
  ...
  it("User cannot lock for more than two years (104 weeks)", async () => {
    ...
    await expect(
      stakingVe.connect(alice).stake(SECONDS_IN_A_YEAR * 2 + 1, 10, false)
  ...
  it("User cannot create lock with more tokens then they have", async () => {
    ...
    await expect(
      stakingVe.connect(alice).stake("1000", "11", false)
  ...
  it("User can increase the duration of a pre-existing lock, up to the max duration of two years", async () => {
    ...
    await expect(
      stakingVe.connect(alice).stake(100, 0, false)
    ).to.be.revertedWith(">2 years");
  });
  ...
  it("User can autocompound their lock", async () => {
    ...
    for (let i = 0; i < 100; ++i) {
      ...
      console.log(
        "Day %d, Balance: %d Power: %d, Wallet: %d, Interest: %d",
        i,
        balance,
        power,
        inWallet,
  ...
  it("User cannot decrease their lock time by means of overflow", async () => {
    ...
    for (let i = 0; i < 256; ++i) {
      ...
      if (duration + extension > SECONDS_IN_A_YEAR * 2) {
        await expect(stakingVe.connect(alice).stake(extension, 0, false)).to.be
          .reverted;
      } else {
  ...
  it("Governance can withdraw tokens other than the staked one", async () => {
    ...
    await coaxialToken
      .connect(alice)
      .transferFrom(aliceAddr, stakingVeAddr, 1000);
